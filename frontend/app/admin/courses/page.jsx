          ...
          <div style={{ background: "var(--card)", padding: "2rem", borderRadius: "0.75rem", width: 600, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "20px" }}>
              <div>
                ...
                <p style={{ margin: "8px 0", fontSize: 14 }}><strong style={{ display: "inline-block", width: "120px" }}>Trạng thái:</strong> <span className="badge" style={{ background: viewingCourse.status === 'Đang mở' ? '#dcfce7' : '#fee2e2', color: viewingCourse.status === 'Đang mở' ? '#16a34a' : '#dc2626' }}>{viewingCourse.status}</span></p>
          ...
          <div style={{ background: "var(--card)", width: "100%", maxWidth: "800px", borderRadius: "12px", overflow: "hidden", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "24px", overflowY: "auto" }}>
              ...
              <form id="courseForm" onSubmit={handleSave} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                ...
                <div className="form-group">
                  <label className="form-label" style={{ fontSize: 14 }}>Trạng thái đăng ký</label>
                  <select className="form-input" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}>
                    <option value="Đang mở">Đang mở (Cho phép ĐK)</option>
                    <option value="Đã đóng">Đã đóng (Không cho phép ĐK)</option>
                  </select>
                </div>
              </form>
            </div>
