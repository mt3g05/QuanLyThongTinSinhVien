...
class GradeService {
  async processImport(file, department_id, cohort_id, major_id, class_id, semester) {
    ...
    if (validRecords.length > 0) {
      // [FIX TASK-06] Lọc bỏ các bản ghi đã được duyệt để không ghi đè
      const existingApproved = await query(
        `SELECT student_id, course_id, semester FROM grades
         WHERE status = 'Đã duyệt'
         AND (student_id, course_id, semester) IN (
           ${validRecords.map(() => '(?,?,?)').join(',')}
         )`,
        validRecords.flatMap(r => [r[0], r[1], r[2]])
      );
      const approvedSet = new Set(
        existingApproved.map(g => `${g.student_id}_${g.course_id}_${g.semester}`)
      );
      const insertableRecords = validRecords.filter(
        r => !approvedSet.has(`${r[0]}_${r[1]}_${r[2]}`)
      );
      const skippedApprovedCount = validRecords.length - insertableRecords.length;
      if (skippedApprovedCount > 0) {
        errors.push(`Bỏ qua ${skippedApprovedCount} bản ghi vì điểm đã được duyệt (không cho phép ghi đè)`);
      }
      ...
      const placeholders = insertableRecords.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ');
      const flatParams = insertableRecords.flat();
      ...
      try {
        ...
        const regRecords = insertableRecords.map(r => [r[0], r[1], r[2], 'Đã xác nhận']);
        ...
        const uniqueCourseIds = [...new Set(insertableRecords.map(r => r[1]))];
        ...
        successCount = insertableRecords.length;
      } catch (err) {
