...
exports.getCourses = async (req, res) => {
  try {
    const { page, limit, search, department_id, major_id, class_id } = req.query;
    ...
    const totalItems = Array.isArray(countResult) ? countResult[0]?.total : countResult?.total || 0;
    ...
    const dataQuery = `
      ...
      LIMIT ? OFFSET ?
    `;
    const courses = await query(dataQuery, [...queryParams, Number(pageSize) || 10, Number(offset) || 0]);

    const pagination = getPagingData(totalItems, Number(currentPage) || 1, Number(pageSize) || 10);
...
exports.createCourse = async (req, res) => {
  try {
    const { 
      ...
      instructor_id, description, max_students, semester, type, status
    } = req.body;
...
exports.updateCourse = async (req, res) => {
  try {
    ...
    // [FIX TASK-09] Không cho phép client tự sửa current_students trực tiếp
    delete updateData.current_students;
    ...
    // [FIX TASK-09] Whitelist các field được phép cập nhật
    ...
    for (const [key, value] of Object.entries(updateData)) {
      if (!ALLOWED_FIELDS.includes(key)) continue; // bỏ qua field không cho phép
      updateFields.push(`${key} = ?`);
      updateParams.push(value === '' ? null : value);
    }

    if (updateFields.length === 0) {
      return ApiResponse.badRequest(res, 'Không có dữ liệu hợp lệ để cập nhật');
    }
...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    const hasActiveRegistrations = await queryOne('SELECT id FROM registrations WHERE course_id = ? AND status != "Đã hủy" LIMIT 1', [id]);
    if (hasActiveRegistrations) return ApiResponse.badRequest(res, 'Không thể xóa: Đã có sinh viên đăng ký môn học này.');

    // Remove any cancelled registrations before deleting course to satisfy foreign key constraints
    await query('DELETE FROM registrations WHERE course_id = ?', [id]);
